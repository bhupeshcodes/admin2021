...
const DriverAutocompleteWrapper = (props) => {
  const [options, setOptions] = useState([]);
  const {  SetDriver } = useContext(mainContext);
  const { value } = props;
    ...
    <Autocomplete
      ...
      renderOption={(option) => {
        const AssignTask2 = props.AssignTask.filter((Assign) => {
          return Assign.Driver === option.username;
        });
        ...
        return (
          <React.Fragment>
            ...
            {AssignTask2.filter((Assign) => {
              return (
                Assign.date.split(" ")[0] === value.date.split(" ")[0]
              );
            }).map((task) => (
              <Button
                variant="outlined"
...
export default DriverAutocompleteWrapper;
